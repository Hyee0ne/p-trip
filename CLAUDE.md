...
3. **비평가**: 별점·후기·랭킹 필드를 스키마에 만들지 않는다.
   ...
   - ⚠ **근거가 다르면 문구도 달라야 한다.** 절대 순위로 뽑힌 길에
     '최근에도 함께 찾는 곳이 많아요'를 붙이면 거짓말이다 — 그 길은 최근 두 시점 자료가 없다.
     `routeNotePopular = '함께 찾는 곳이 많아요'` 를 따로 쓴다.
   - ⚠ **문구는 데이터가 보장하는 만큼만** (2026-09-16). 연관 관광지 API는 '같은 방문자가 함께 찾은 곳'의 순위다.
     방문자 수·증가량·이동 순서·차량은 모른다 — '발길이 늘었다'·'늘었어요'·'다녀간 사람이 많다'·'들른 차들은 다음에'는
     전부 과장이라 지웠다. 두 시점 순위 비교는 **정렬 참고**로만 쓰고, 문구는 '최근 기준'이라는 것만 말한다.
...
- **지도**: **Apple MapKit** (apple_maps_flutter). 2026-09-09 카카오맵(kakao_map_sdk)에서 교체.
  ...
  ⚠ **`AppleMap` 을 매 프레임 다시 빌드하지 말 것** (2026-09-13 발견 탭 렉). 플러그인이 rebuild 마다
    폴리라인 전부를 플랫폼에 다시 보낸다 — 51선 1만 점이 시트를 끄는 프레임마다 채널을 건넜다.
    지도 패널은 `AppleMap` 인스턴스를 들고 있다가 선·마커가 바뀔 때만 새로 만들고, 시트 높이는
    `ValueNotifier` 로 흘린다. 지도 위에 무언가 얹을 때 이 구조를 깨지 말 것 (SCREENS.md CO-07).
