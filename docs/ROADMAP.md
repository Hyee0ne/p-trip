...
- [x] **데이터 실측 스파이크 (0.5일)** — 2026-08-29 실행. 결과:
      ...
      · ❌ 연관 관광지 — 경로는 맞으나(`TarRlteTarService1/areaBasedList1`)
        **403 SERVICE_KEY_IS_NOT_REGISTERED** → 활용신청 필요 (data.go.kr 15128560)
      · ❌ 전통시장 표준데이터 — uddi가 데이터셋마다 달라 추측 불가. 마이페이지 요청주소 필요
- [ ] ~~착수 전 필수 확인~~ (위에서 완료):
