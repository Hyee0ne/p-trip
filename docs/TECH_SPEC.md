...
### 3.8 별 보기 좋은 밤 (2026-08-29 추가) — **미구현, 노출 지점 없음 (2026-09-07)**

⚠ 아래 설계는 **만들지 않았다.** 만든 건 `moonless`/`eventTitle` 한 줄 문장뿐이었고,
  그건 CO-06·MY-02에서 **분위기만 말하고 어떤 장소로도 이어지지 않았다** — 그래서 뺐다.
  천문연 데이터가 남아 있는 곳은 **레이더 일몰 카드(§3.1 타이밍 가중치)** 하나다.
  거기서는 전망 좋은 곳이라는 장소와 일몰 −60~−20분이라는 시점이 함께 있어 "들르라"고 말할 수 있다.
  아래 곱(장소 × 그날 밤)을 실제로 구현하면 그때 되살린다. 데이터·테이블·RPC는 그대로 둔다.
...
| 천문연 천문현상 정보 | 유성우·월식·슈퍼문 — **앱에서 읽는 곳 없음** (§3.8 미구현). 테이블만 채워둔다 | data.go.kr (B090041) | pipeline |
