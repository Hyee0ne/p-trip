...
5. **진출로 역산 — 거리 기준** (2026-08-30 개정):
   `ahead_km = (spot.exit_frac - now_frac) * route.total_km`
   - `now_frac` = 현 위치를 ST_LineLocatePoint 한 값. 0 이하면 이미 지나친 것 → 후보 제외
   - **진행 방향 반경 5km 안이면 띄운다** (`_aheadKm`, radar_screen.dart)
   - ~~3~7분 구간~~ → **폐기.** 속도를 타서 같은 길이라도 막히면 코앞만, 뻥 뚫리면
     한참 먼 것이 떴다. 거리라야 "여기서 N km 안"이 말이 된다
   - ⚠ 라우팅 API를 쓰지 않는다. 노선 형상만으로 산출한다 — 속도조차 안 쓴다
