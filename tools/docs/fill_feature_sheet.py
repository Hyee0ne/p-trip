...
PLAN = [
    '• 여행기 공유 → 「남이 달린 길」 — 내 여행기(지나온 길·들른 곳)를 공유하면 다른 사람이 그 길을 그대로 고를 수 있다. 계정을 붙여 내 여행기·찜을 기기 밖에 보관하고, 팔로우한 사람의 새 여행기가 홈 지도에 뜬다. 별점·댓글 대신 "N명이 달림 · N명이 멈춤"의 흔적만',
    '• 사용자 정차 흔적 기반 큐레이션 — 익명 정차 기록을 모아 "이 길에서 사람들이 실제로 멈춘 곳"을 만든다. 관광공사 연관 관광지 두 시점에 우리 사용자 흔적을 더해 노선 큐레이션의 두 번째 축으로',
    '• 음성 조작 — 카드에 말로 답하기(찜·들르기·넘기기). 읽어주기만 있는 지금 레이더를 손을 떼지 않는 레이더로',
    '• 시간에 걸린 발견 확대 — 축제 기간(관광공사 행사정보)·야간 개장·계절 발견(단풍·벚꽃·해돋이)',
    '• 국도 여행 공공데이터 확장 — 농산물 직판장·로컬푸드·지역 축제를 국도 선형에 겹치고, 노선을 구간으로 잘라 코스를 자동 생성',
]
