...
INTRO = {
    ...
    'type': '앱 서비스 (iOS · App Store 1.0.2 출시, 1.0.3 심사 중 · 2026.09)',
    'summary': '목적지 대신 국도와 방향만 고르면, 달리는 동안 앞쪽 5km 안의 "지금 아니면 못 보는" 발견(오늘 서는 5일장·40분 뒤 일몰·확인된 맛집)을 카드 한 장과 음성으로 알려주는 발견 레이더 앱',
    'why': [
        '• 고속도로 중심 이동은 빠르지만 아무것도 남기지 않는다. 국도변에는 오늘만 서는 장, 일몰 명소, 이름 없는 물회 골목이 있지만 대부분 지나치고 나서야 안다.',
        '• 내비게이션은 목적지를 먼저 묻고, 관광 앱은 출발 전에 검색과 계획을 요구한다. "달리는 동안" 관광정보를 건네는 채널이 비어 있었다.',
        '• 한국관광공사 관광정보에는 시간에 걸린 정보(장날·축제 기간·영업시간·사진)가 이미 있다. 이를 국도 선형과 현재 위치·진행 방향에 겹치면 "지금 이 길 앞에 무엇이 있는가"로 바꿀 수 있다.',
        '• 목적지 없는 국도 여행은 대도시 밖 소도시·전통시장·자연 관광지로 발길을 분산시킨다 — 관광데이터로 지역 관광을 살리는 일과 맞닿는다.',
    ],
}

FEATURES_LIST = [
    '1. 지도에서 길 고르기 — Apple 지도 위에 국도 51선 전부. 내 주변에서 탈 수 있는 길과 노선별 큐레이션 한 줄(「오늘 이 길에 장이 3곳 서요」 「이 길로 다녀간 사람이 많아요」)',
    '2. 방향만 정하고 출발 — 목적지·도착 시각을 묻지 않는다. 북/남(동/서)만 고르면 티맵·애플 지도로 길안내를 넘긴다 (앱은 내비게이션이 아니다)',
    '3. 발견 레이더 — 현재 위치 + 진행 방향 반경 5km의 검증된 스팟(영업정보·사진 확인)을 전면 카드 한 장으로. 장날·일몰·마감 임박을 우선. 넘기기 / 들르기 / 찜',
    '4. 소리와 알림 — 내비를 켜둔 채여도 음성으로 먼저(내비 음성을 끊지 않고 잠깐 낮춘다). 놓치면 알림에 남고, 누르면 그 발견의 상세로',
    '5. 들른 뒤 이어가기 — 차가 2분 서 있으면 「여기서 앞쪽으로」 앞쪽 10km 후보 3곳. 여행 중 다른 국도로 갈아타도 여행과 기록은 이어진다',
    '6. 여행기 — 하루가 「N번 국도에서 생긴 일」 한 편으로. 지나온 길 포스터·들른 곳 타임라인·공유 카드(시작·끝 300m 자동 삭제)·국도 51선 수집 진행률',
    '',
    '원칙: 별점·후기 없음 · 예약·결제 없음 · 로그인 없음(찜·여행기는 기기 안) · 광고·추적 없음 · 정확한 GPS 좌표는 기기를 떠나지 않음',
]
