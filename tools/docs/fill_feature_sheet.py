...
INTRO = {
    ...
    'why': [
        '• 반복되는 불편',
        '- 목적지 없이 드라이브를 떠나도 어디에 들를지는 계속 결정해야 함',
        '- 주변 장소 검색은 되지만, 진행 방향과 장날·일몰 같은 시간 조건까지 함께 보려면 여러 정보를 찾아봐야 함',
        '• 커지는 관심',
        '- 목적지 없이 떠나는 예능 「풍향중」의 흥행처럼, 국도에서 우연히 만나는 새로움에 대한 관심이 커짐',
        '• 관광데이터의 가능성',
        '- 관광공사 관광정보의 영업시간·휴무일·장날·사진·연관 관광지를 국도 선형과 현재 위치·진행 방향에 결합해 "지금 이 길 앞에 무엇이 있는가"로 전환',
        '• 기획 방향',
        '- 목적지는 묻지 않되, 지금 지나가는 길에서 들를 이유가 있는 장소를 먼저 알려 탐색을 줄임',
        '- 휴무일·장날 정보로 헛걸음 가능성을 줄이고, 정보가 없거나 불확실한 장소는 노출하지 않음',
        '• 기대효과와 검증',
        '- 목적지까지 통과하던 여행자가 계획에 없던 지역의 장소에 한 번 더 멈추게 함',
        '- 초기 실증: 추천 후 「들르기」 선택과 정차 감지에 따른 방문 여부를 기기 안에서 집계하고, 여행 후 질문으로 계획에 없던 방문을 확인 (위치 데이터는 기기 밖으로 나가지 않음, 사용자 동의 기반)',
    ],
}
...
FEATURES_LIST = [
    ...
    '- 노선별 큐레이션 한 줄: 오늘 장이 서는 길, 함께 찾는 곳이 늘어난 길(연관 관광지 두 시점 비교)',
    '2. 발견 레이더',
    '- 주행 중: 현재 위치·진행 방향 반경 5km에서 영업정보와 사진을 확인할 수 있는 장소만 카드 한 장으로. 거리는 직선거리(도로 거리·소요 시간 아님)',
    '- 정보가 없거나 불확실한 장소는 노출하지 않음. 정차 2분이면 「여기서 앞쪽으로」 앞쪽 10km 후보 3곳, 다른 국도로 갈아타도 기록 유지',
    '3. 오늘만 볼 수 있는 발견 우선',
...
FLOWS = [
    ...
    },
    {
        ...
        'shots': ['4_radar.png', ('case-card.png', (0.34, 0.82)), 'onboard3-t6.png', 'myset2-t8.png'],
        'steps': [
            ['① 레이더', '- 내 위치 기준 앞쪽 스팟(유형별 색)', '- 경로를 따르지 않아 길을 바꿔도 동작'],
            ['② 장날 카드 (확대)', '- 장날엔 「오늘이 마침 북평민속오일장이에요」, 평일엔 「다음 장은 N일 뒤」', '- 「여기서 약 N km」는 직선거리'],
    ...
    },
    {
        ...
        'steps': [
            ...
            ['② 핸드오프 (확대)', '- 「북평민속오일장」 → 티맵 / 애플 지도', '- 안내 중에도 목적지 변경'],
    ...
    },
    {
        ...
        'steps': [
            ...
            ['② 여행기', '- 「7번 국도에서 생긴 일」', '- 실제 GPS 선 포스터 + 들른 곳(북평민속오일장)'],
...
TOUR_APIS = [
    ...
    ('관광지별 연관 관광지 정보 서비스(TarRlteTarService1) — areaBasedList1 (baseYm 지정)',
     ['같은 방문자가 함께 찾은 관광지의 순위 데이터. 이 관광지와 연관성이 높은 주변 관광지를 함께 제안하는 데 활용',
      '기준연월 두 시점(예: 202603·202606)의 순위 변화를 노선 큐레이션 한 줄(「요즘 함께 찾는 곳이 늘었어요」)의 근거로 사용',
      '방문자 수·이동 순서·차량 이용은 이 데이터로 알 수 없어 그렇게 표현하지 않음. 순위는 정렬 보조로만, 별점·후기 미사용']),
]
