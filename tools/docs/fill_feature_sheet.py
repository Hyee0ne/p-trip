...
DIFF = [
    '• 즉흥 여행이라는 트렌드를 제품으로 — 「풍향중」처럼 목적지 없이 떠나는 문화에 맞춘 유일한 앱. 즉흥은 그대로 두고 리스크(휴무·비장날·사진 없는 곳)만 관광데이터로 걸러 "휴일을 버리지 않는 즉흥"을 만든다',
    '• 시장에 없는 시점 — 출발 전 계획(여행 플랫폼)도, 목적지 검색(내비)도 아닌 "달리는 동안" 관광정보를 먼저 건네는 채널. 국도와 방향만 고르면 정보는 가면서 나온다',
    '• 같은 관광공사 데이터를 이동 맥락으로 재해석 — 장날·일몰·축제 기간을 현재 위치·진행 방향·시각에 겹쳐 "지금 아니면 없는 것"을 우선한다. 천문연·전통시장·국토부 데이터를 국도 선형 위에서 결합',
    '• 내비의 대체가 아니라 내비 위의 레이어 — 길안내는 티맵·애플 지도가, 발견은 우리가. 안내 중에도 「들르기」로 목적지가 바뀐다. 경쟁이 아니라 공존',
    '• 신뢰의 근거가 다르다 — 별점·리뷰 대신 영업정보·사진 검증과 이동 흔적(연관 관광지 두 시점 변화율). 관광지가 평가되는 대신 발견된다',
    '• 하루가 저절로 기록된다 — 일정표를 짜는 앱이 아니라 달린 뒤 여행기·포스터·국도 51선 수집이 남는 앱. 계정·광고·추적 없이, 정확한 GPS 좌표는 기기를 떠나지 않는다',
]
PLAN = [
    '• 여행기 공유 → 「남이 달린 길」 — 내 여행기(지나온 길·들른 곳)를 공유하면 다른 사람이 그 길을 그대로 고를 수 있다. 계정을 붙여 여행기·찜을 기기 밖에 보관하고, 팔로우한 사람의 새 여행기가 홈 지도에 뜬다. 별점·댓글 대신 "N명이 달림 · N명이 멈춤"의 흔적만',
    '• 사용자 정차 흔적 기반 큐레이션 — 익명 정차 기록으로 "이 길에서 사람들이 실제로 멈춘 곳". 리뷰 없이도 신뢰가 쌓이는 장치로, 관광공사 연관 관광지 두 시점에 우리 흔적을 더한다',
    '• 음성 조작 — 카드에 말로 답하기(찜·들르기·넘기기). 읽어주기만 있는 지금 레이더를 손을 떼지 않는 레이더로',
    '• 시간에 걸린 발견 확대 — 축제 기간(관광공사 행사정보)·야간 개장·계절 발견(단풍·벚꽃·해돋이)',
    '• 데이터 갱신·확장 — 수집 파이프라인 상시화로 갱신 지연을 줄이고, 농산물 직판장·로컬푸드·지역 축제로 소도시의 빈칸을 채운다. 노선을 구간으로 잘라 코스 자동 생성',
    '• Android 확장 — 유지해 둔 코드 분기로 포팅해 더 많은 운전자에게',
]
