...
INTRO = {
    ...
    'summary': [
        '목적지 없이 국도를 달려도 휴일을 버리지 않도록, 관광데이터로 검증된 발견을 지나치기 전에 알려주는 즉흥 여행 앱',
        '국도와 방향만 선택하면 앞쪽의 오늘 서는 장·일몰·확인된 맛집을 카드 한 장으로 제공',
    ],
    'why': [
        '• 즉흥 여행의 확산: 「풍향중」처럼 목적지 없이 떠나는 여행이 유행하며, 국도에서 우연히 만나는 새로움에 대한 수요 증가',
        '• 즉흥 여행의 리스크: 지도책만으로 나서면 문 닫은 식당, 장이 서지 않는 날, 지나친 뒤 알게 되는 명소 등으로 휴일을 낭비할 위험이 큼',
        '• 관광데이터의 가능성: 한국관광공사 관광정보의 영업시간·휴무일·장날·사진·연관 관광지를 국도 선형과 현재 위치·진행 방향에 결합하면 "지금 이 길 앞에 무엇이 있는가"로 전환 가능',
        '• 기획 방향: 목적지는 묻지 않되 우연을 데이터로 뒷받침하는 앱. 국도에서 만난 우연이 하루의 여행기로 남고, 발길이 소도시·시장·자연으로 분산되는 효과 기대',
    ],
}

FEATURES_LIST = [
    '1. 길 선택과 출발: Apple 지도 위에 국도 51선 전체 표시, 내 주변에서 탈 수 있는 길과 노선별 큐레이션 한 줄 제공. 목적지·도착 시각을 묻지 않고 방향(북/남, 동/서)만 선택하면 티맵·애플 지도로 길안내 연결 (앱 자체는 내비게이션이 아님)',
    '2. 발견 레이더: 현재 위치와 진행 방향 반경 5km에서 영업정보·사진이 확인된 스팟만 전면 카드 한 장으로 제시. 별점·후기를 쓰지 않고 데이터 완성도로 신뢰 확보. 장날·일몰·마감 임박 우선. 넘기기 / 들르기 / 찜',
    '3. 소리와 알림: 내비 실행 중에도 음성으로 우선 안내(내비 음성을 끊지 않고 일시적으로 낮춤). 놓친 발견은 알림에 보관, 알림 선택 시 상세 화면으로 이동',
    '4. 들른 뒤 이어가기: 정차 2분 시 「여기서 앞쪽으로」 앞쪽 10km 후보 3곳 제시. 주행 중 다른 국도로 갈아타도 여행과 기록 유지',
    '5. 여행기: 하루를 「N번 국도에서 생긴 일」 한 편으로 자동 기록. 지나온 길 포스터, 들른 곳 타임라인, 공유 카드(시작·끝 300m 자동 삭제), 국도 51선 수집 현황',
]
...
FLOWS = [
    {
        'title': '길 선택 · 방향 결정 · 출발',
        'desc': '목적지가 아닌 길을 선택. 홈 지도에 국도 51선을 모두 표시하고 방향만 정하면 출발. 길안내는 티맵·애플 지도로 연결',
        'shots': ['1_home.png', 'home_expanded.png', '2_depart.png', 'handoff-t16.png'],
        'steps': [
            '① 홈 = 지도. 국도 51선 표시, 「여기서 탈 수 있는 길」과 노선별 큐레이션 한 줄',
            '② 시트를 올리면 「국도 51선」 전체. 남북(홀수)·동서(짝수) 필터',
            '③ 노선 선택 → 「여기서 어느 쪽으로 갈까요?」 북/남만 선택. 목적지 입력 없음',
            '④ 티맵 / 애플 지도로 핸드오프. 앱 자체의 턴바이턴·ETA 없음',
        ],
    },
    {
        'title': '발견 레이더 · 전면 카드',
        'desc': '현재 위치·진행 방향 반경 5km에서 영업정보·사진이 확인된 스팟만 카드 한 장으로 제시. 장날·일몰·마감 임박 우선',
        'shots': ['4_radar.png', '3_card.png', '5_spot.png', 'radar-t26.png'],
        'steps': [
            '① 레이더: 내 위치 기준 앞쪽 스팟(유형별 색). 경로를 따르지 않으므로 길을 바꿔도 동작',
            '② 발견 카드: 「오늘이 마침 북평민속오일장이에요 · 여기서 약 1.2km」 + 넘기기 / 들르기 / 찜',
            '③ 상세: 관광공사 사진·개요·영업정보. 별점 없이 「네이버 후기 보기」 링크만 제공',
            '④ 「들르기」 선택 시 티맵 목적지 변경, 「오늘 들른 곳」 자취에 사진으로 기록',
        ],
    },
    {
        'title': '들른 뒤 이어가기 · 길 바꾸기',
        'desc': '식사 후 다음 행선지를 정하는 순간 지원. 정차 2분이면 앞쪽 10km 후보 제시, 다른 국도로 갈아타도 기록 유지',
        'shots': ['next2-t17.png', 'next2-t22.png', 'switch-t16.png', 'radar-t14.png'],
        'steps': [
            '① 들른 뒤 2분 정차 시 「여기서 앞쪽으로」 자동 표시',
            '② 앞쪽 10km 후보 3곳 + 「그냥 7번 국도로 돌아가기」',
            '③ 상단 뱃지 선택 → 「길을 바꿀까요?」 인근 국도로 전환',
            '④ 여행은 하루 단위, 국도는 구간 단위로 연결(43 → 6번 국도). 거리·들른 곳 유지',
        ],
    },
    {
        'title': '여행기 · 공유 포스터 · 국도 51선 수집',
        'desc': '「오늘 여행 마치기」 선택 시 지나온 길과 들른 곳을 한 편의 여행기로 자동 생성. 포스터 한 장으로 공유, 지나온 국도를 51선 기준으로 누적',
        'shots': ['trip-t16.png', 'share_card_8stops.png', 'my-t8.png', 'radar-t14.png'],
        'steps': [
            '① 「EP.6 — 43번 국도에서 생긴 일」: 실제 GPS 선 포스터 + 들른 곳 타임라인',
            '② 공유 카드: 지도·들른 곳 번호 목록·「들른 발견 N」. 시작·끝 300m 자동 삭제',
            '③ 마이: 여행기 목록·찜·국도 51선 수집 현황·설정',
            '④ 레이더의 「오늘 여행 마치기」 한 번으로 하루 종료',
        ],
    },
    {
        'title': '소리 · 알림 · 설정',
        'desc': '주행 중 화면 대신 소리로 우선 안내, 놓친 발견은 알림에 보관. 위치 권한은 출발 시, 알림 권한은 첫 주행 후 요청',
        'shots': ['onboard3-t6.png', '3_card.png', 'myset2-t8.png', '5_spot.png'],
        'steps': [
            '① 온보딩 마지막 장: 「앞쪽에 뭐가 있는지, 카드 한 장으로」, 소리·알림 안내',
            '② 카드 문장을 음성으로 낭독. 내비 음성을 끊지 않고 일시적으로 낮춤',
            '③ 설정: 「앱을 꺼둬도 알림」(백그라운드 위치 옵트인), 「발견 간격」 자주·보통·가끔, 데이터 출처. 낭독은 레이더의 스피커 버튼',
            '④ 알림 선택 시 해당 발견의 상세 화면으로 이동',
        ],
    },
]
...
TOUR_APIS = [
    ('한국관광공사 국문 관광정보 서비스(KorService2) — 지역기반 관광정보 조회 areaBasedList2',
     '전국 관광지·음식점·문화시설·쇼핑(5일장·상설시장) 목록을 시군구 단위로 수집, 국도 51선 선형 반경 회랑 내 스팟만 적재(19,957건). 레이더·발견 카드의 후보 데이터'),
    ('국문 관광정보 — 공통정보 조회 detailCommon2 · 소개정보 조회 detailIntro2',
     '개요·영업시간·휴무일·장날·전화번호 수집. 신뢰도 게이트(영업정보·사진 확인)의 근거와 카드 본문 생성(예: 「3·8일에만 서는 장이라, 다음 장은 5일 뒤예요」)'),
    ('국문 관광정보 — 이미지정보 조회 detailImage2',
     '관광공사 사진을 전면 카드·스팟 상세·「오늘 들른 곳」 자취에 원본 그대로(워터마크 포함) 사용. 사진이 없는 스팟은 카드로 노출하지 않음'),
    ('국문 관광정보 — 법정동코드 조회 ldongCode2 · 위치기반 관광정보 조회 locationBasedList2',
     '시군구 코드 순회 수집과 연관 관광지 이름 매칭의 지역 키. 위치기반 조회는 회랑 보강·검증에 활용'),
    ('관광지별 연관 관광지 정보 서비스(TarRlteTarService1) — areaBasedList1 (baseYm 지정)',
     '기준연월 두 시점(예: 202603·202606)을 비교해 "요즘 더 가는 곳"의 변화율 산출. 노선 큐레이션(「요즘 이 길에 발길이 늘었어요」)과 「들른 차들은 다음에 ○○로 갔어요」의 근거. 두 시점이 없는 노선은 해당 월 연관 순위로 보조하되 문구 구분(「이 길로 다녀간 사람이 많아요」). 별점·후기 미사용'),
]

OTHER_DATA = [
    ('한국천문연구원 출몰시각 정보 OpenAPI (RiseSetInfoService)',
     '전망 스팟이 있는 0.1° 격자 × 30일의 일몰 시각을 사전 캐시. 일몰 40분 전 뷰포인트 카드·알림(「곧 추암 촛대바위에 해가 져요」)의 기준'),
    ('소상공인시장진흥공단 전국전통시장표준데이터 (파일, CSV)',
     '장날(3·8일 등)을 관광공사 시장 스팟에 결합. 「오늘이 마침 장날」 카드와 다음 장 안내. 사진·개요가 있는 관광공사 스팟에만 결합(신규 스팟 생성 없음)'),
    ('국토교통부 일반국도 도로중심선 (파일, SHP)',
     '국도 51선 선형. 홈 지도 폴리라인, 회랑 판정(스팟 적재 기준), 진출점 계산, 여행기의 국도 51선 수집(지나온 점 맵매칭)에 활용'),
]

DIFF = [
    '• 즉흥 여행 트렌드의 제품화: 「풍향중」처럼 목적지 없이 떠나는 여행 문화에 맞춘 앱. 즉흥은 유지하고 리스크(휴무·비장날·사진 없는 곳)만 관광데이터로 제거',
    '• 시장에 없는 시점: 출발 전 계획(여행 플랫폼)도 목적지 검색(내비)도 아닌 "주행 중" 관광정보 제공. 국도와 방향만 선택하면 정보는 이동 중에 제공',
    '• 관광공사 데이터의 이동 맥락 재해석: 장날·일몰·축제 기간을 현재 위치·진행 방향·시각에 결합해 "지금 아니면 없는 것"을 우선. 천문연·전통시장·국토부 데이터를 국도 선형 위에서 결합',
    '• 내비의 대체가 아닌 내비 위의 레이어: 길안내는 티맵·애플 지도, 발견은 본 앱. 안내 중에도 「들르기」로 목적지 변경 가능',
    '• 신뢰 근거의 차별화: 별점·리뷰 대신 영업정보·사진 검증과 이동 흔적(연관 관광지 두 시점 변화율) 활용',
    '• 자동 기록: 일정표를 작성하는 앱이 아니라 주행 후 여행기·포스터·국도 51선 수집이 남는 앱. 계정·광고·추적 없음, 정확한 GPS 좌표는 기기 내 보관',
]
PLAN = [
    '• 여행기 공유 → 「남이 달린 길」: 여행기(지나온 길·들른 곳) 공유로 다른 사용자가 같은 길을 선택 가능. 계정 도입으로 여행기·찜 보관과 팔로우 지원. 별점·댓글 없이 "N명이 달림 · N명이 멈춤" 흔적만 표시',
    '• 사용자 정차 흔적 기반 큐레이션: 익명 정차 기록으로 "실제로 멈춘 곳" 산출. 리뷰 없이 신뢰를 축적하는 장치로, 관광공사 연관 관광지 두 시점에 사용자 흔적 추가',
    '• 음성 조작: 카드에 음성으로 응답(찜·들르기·넘기기). 낭독 전용인 현재 레이더를 핸즈프리로 확장',
    '• 시간 기반 발견 확대: 축제 기간(관광공사 행사정보)·야간 개장·계절 발견(단풍·벚꽃·해돋이)',
    '• 데이터 갱신·확장: 수집 파이프라인 상시화로 갱신 지연 축소, 농산물 직판장·로컬푸드·지역 축제로 소도시 공백 보완, 노선 구간 단위 코스 자동 생성',
    '• Android 확장: 유지 중인 코드 분기를 활용한 포팅',
]
...
set_cell(t.cell(0, 1), FEATURES_LIST, size=15)
...
set_cell(t.cell(0, 1), DIFF, size=11)
set_cell(t.cell(1, 1), PLAN, size=11)
